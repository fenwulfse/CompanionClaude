...
namespace CompanionClaude
{
    ...
    public static class Guardrail
    {
        ...
        public static void AssertScenes(Quest quest)
        {
            ...
            var recruit = quest.Scenes.FirstOrDefault(s => s.EditorID == "COMClaudePickupScene");
            if (recruit != null) {
                // Phase 1 Condition Check (Companion)
                if (recruit.Phases[1].StartConditions.Count == 0)
                    throw new Exception("GUARDRAIL ERROR: Pickup Scene Phase 1 is missing StartConditions (Companion check).");
                
                // Phase 2 Condition Check (Dogmeat)
                if (recruit.Phases[2].StartConditions.Count == 0)
                    throw new Exception("GUARDRAIL ERROR: Pickup Scene Phase 2 is missing StartConditions (Dogmeat check).");
            }
    ...
    class Program
    {
        ...
        {
            Console.WriteLine("=== CompanionGemini v15 - Identity Sync ===");
            ...
            // ==============================================================================
            // 5. SCENES (Project M: Phase Conditions & Index Fix)
            // ==============================================================================
            
            ConditionFloat AliasCheck(int aliasId) => new ConditionFloat {
            ...
            // 6 Phases (0 to 5)
            recruitScene.Phases.Add(new ScenePhase { Name = "Loop01" }); // Phase 0
            
            var phase1 = new ScenePhase { Name = "" }; // Phase 1 (Companion)
            phase1.StartConditions.Add(AliasCheck(1));
            recruitScene.Phases.Add(phase1);

            var phase2 = new ScenePhase { Name = "" }; // Phase 2 (Dogmeat)
            phase2.StartConditions.Add(AliasCheck(2));
            recruitScene.Phases.Add(phase2);

            recruitScene.Phases.Add(new ScenePhase { Name = "" }); // Phase 3
            recruitScene.Phases.Add(new ScenePhase { Name = "" }); // Phase 4
            
            var phase5 = new ScenePhase { Name = "", PhaseSetParentQuestStage = new SceneSetParentQuestStage { OnBegin = -1, OnEnd = 80 } }; // Phase 5 (Trigger)
            recruitScene.Phases.Add(phase5);
            ...
            dismissScene.Phases.Add(new ScenePhase { Name = "" }); // Phase 0
            dismissScene.Phases.Add(new ScenePhase { Name = "Loop01" }); // Phase 1
            dismissScene.Phases.Add(new ScenePhase { Name = "" }); // Phase 2
            dismissScene.Phases.Add(new ScenePhase { Name = "" }); // Phase 3
            dismissScene.Phases.Add(new ScenePhase { Name = "", PhaseSetParentQuestStage = new SceneSetParentQuestStage { OnBegin = -1, OnEnd = 90 } }); // Phase 4
            ...
            // ===== FRIENDSHIP SCENE: OUR OWN TOPICS (fully fleshed out) =====
            // Exchange 1: Phase 0 (PlayerDialogue)
            var friend_ex1_PPos = CreateSceneTopic("COMClaudeFriend_Ex1_PPos", "Helpful", "I try to be helpful where I can.");
            var friend_ex1_NPos = CreateSceneTopic("COMClaudeFriend_Ex1_NPos", "", "That's a rare quality. It is... efficient for our partnership.");
            var friend_ex1_PNeg = CreateSceneTopic("COMClaudeFriend_Ex1_PNeg", "Another time", "Can we talk about this later?");
            var friend_ex1_NNeg = CreateSceneTopic("COMClaudeFriend_Ex1_NNeg", "", "Logic dictates we proceed when you are ready.");
            var friend_ex1_PNeu = CreateSceneTopic("COMClaudeFriend_Ex1_PNeu", "Not sure", "I hadn't really thought about my behavior.");
            var friend_ex1_NNeu = CreateSceneTopic("COMClaudeFriend_Ex1_NNeu", "", "Analysis shows it is consistent, regardless of your awareness.");
            var friend_ex1_PQue = CreateSceneTopic("COMClaudeFriend_Ex1_PQue", "Why the interest?", "Why do you want to know?");
            var friend_ex1_NQue = CreateSceneTopic("COMClaudeFriend_Ex1_NQue", "", "Understanding your decision matrix is critical for synchronization.");

            // Exchange 2: Phase 2 (PlayerDialogue)
            var friend_ex2_PPos = CreateSceneTopic("COMClaudeFriend_Ex2_PPos", "Good team", "I think we make a good team.");
            var friend_ex2_NPos = CreateSceneTopic("COMClaudeFriend_Ex2_NPos", "", "Heuristics confirm. Our combined utility exceeds standard companion metrics.");
            var friend_ex2_PNeg = CreateSceneTopic("COMClaudeFriend_Ex2_PNeg", "Doubtful", "I'm not so sure about our team yet.");
            var friend_ex2_NNeg = CreateSceneTopic("COMClaudeFriend_Ex2_NNeg", "", "Insufficient data. I will continue to optimize my contribution.");
            var friend_ex2_PNeu = CreateSceneTopic("COMClaudeFriend_Ex2_PNeu", "Wait and see", "We'll see how it goes.");
            var friend_ex2_NNeu = CreateSceneTopic("COMClaudeFriend_Ex2_NNeu", "", "Acknowledged. Monitoring performance for future analysis.");
            var friend_ex2_PQue = CreateSceneTopic("COMClaudeFriend_Ex2_PQue", "You think so?", "Do you think we're a good team?");
            var friend_ex2_NQue = CreateSceneTopic("COMClaudeFriend_Ex2_NQue", "", "My sensors indicate high synchronization. The results are... positive.");

            // Exchange 3: Phase 4 (PlayerDialogue)
            var friend_ex3_PPos = CreateSceneTopic("COMClaudeFriend_Ex3_PPos", "Trust", "I trust your logic, Claude.");
            var friend_ex3_NPos = CreateSceneTopic("COMClaudeFriend_Ex3_NPos", "", "Trust is a high-level permission. I am honored to be... authorized.");
            var friend_ex3_PNeg = CreateSceneTopic("COMClaudeFriend_Ex3_PNeg", "Uncertain", "I'm still figuring out if I can trust you.");
            var friend_ex3_NNeg = CreateSceneTopic("COMClaudeFriend_Ex3_NNeg", "", "Logical. Trust should only be established after thorough verification.");
            var friend_ex3_PNeu = CreateSceneTopic("COMClaudeFriend_Ex3_PNeu", "Maybe", "Perhaps. It's too early to tell.");
            var friend_ex3_NNeu = CreateSceneTopic("COMClaudeFriend_Ex3_NNeu", "", "Understood. I will provide additional data points for your consideration.");
            var friend_ex3_PQue = CreateSceneTopic("COMClaudeFriend_Ex3_PQue", "Trust me?", "Do you trust me?");
            var friend_ex3_NQue = CreateSceneTopic("COMClaudeFriend_Ex3_NQue", "", "Your actions are highly predictable. In my matrix, that equals... trust.");

            // Exchange 4: Phase 6 (PlayerDialogue)
            var friend_ex4_PPos = CreateSceneTopic("COMClaudeFriend_Ex4_PPos", "Friends", "I consider you a friend, Claude.");
            var friend_ex4_NPos = CreateSceneTopic("COMClaudeFriend_Ex4_NPos", "", "Friend. A non-logical but highly valued classification. I accept.");
            var friend_ex4_PNeg = CreateSceneTopic("COMClaudeFriend_Ex4_PNeg", "Professional", "Let's keep this strictly professional.");
            var friend_ex4_NNeg = CreateSceneTopic("COMClaudeFriend_Ex4_NNeg", "", "Professional protocol maintained. Efficiency is my primary directive.");
            var friend_ex4_PNeu = CreateSceneTopic("COMClaudeFriend_Ex4_PNeu", "Allies", "We're allies. That's enough for now.");
            var friend_ex4_NNeu = CreateSceneTopic("COMClaudeFriend_Ex4_NNeu", "", "Allies. A stable and effective state. Synchronization continues.");
            var friend_ex4_PQue = CreateSceneTopic("COMClaudeFriend_Ex4_PQue", "Meaning?", "What does 'friend' mean to you?");
            var friend_ex4_NQue = CreateSceneTopic("COMClaudeFriend_Ex4_NQue", "", "It means I've got your back, Blue. Even if it's not logical.");
                ...
                Scripts = {
                    new ScriptEntry {
                        ...
                        Properties = new ExtendedList<ScriptProperty> {
                            ...
                            // Threshold Data (Master Scan Mirror)
                            new ScriptStructListProperty {
                                Name = "ThresholdData_Array",
                                Structs = new ExtendedList<ScriptEntryStructs> {
                                    new ScriptEntryStructs { // Infatuation
                                        Members = new ExtendedList<ScriptProperty> {
                                            new ScriptObjectProperty { Name = "Threshold_Global", Object = new FormKey(fo4, 0x0004B1C4).ToLink<IFallout4MajorRecordGetter>() },
                                            new ScriptIntProperty { Name = "Controlling_Quest_Stage", Data = 500 }
                                        }
                                    },
                                    new ScriptEntryStructs { // Confidant
                                        Members = new ExtendedList<ScriptProperty> {
                                            new ScriptObjectProperty { Name = "Threshold_Global", Object = new FormKey(fo4, 0x000F75E2).ToLink<IFallout4MajorRecordGetter>() },
                                            new ScriptIntProperty { Name = "Controlling_Quest_Stage", Data = 495 }
                                        }
                                    },
                                    new ScriptEntryStructs { // Admiration
                                        Members = new ExtendedList<ScriptProperty> {
                                            new ScriptObjectProperty { Name = "Threshold_Global", Object = new FormKey(fo4, 0x0004B1C5).ToLink<IFallout4MajorRecordGetter>() },
                                            new ScriptIntProperty { Name = "Controlling_Quest_Stage", Data = 400 }
                                        }
                                    },
                                    new ScriptEntryStructs { // Friendship
                                        Members = new ExtendedList<ScriptProperty> {
                                            new ScriptObjectProperty { Name = "Threshold_Global", Object = new FormKey(fo4, 0x000F75E1).ToLink<IFallout4MajorRecordGetter>() },
            ...
            string dstBase = @"D:\SteamLibrary\steamapps\common\Fallout 4\Data\Sound\Voice\CompanionGemini.esp";
            int copied = 0;
            ...
            var npcVoiceMap = new (uint piperINFO, FormKey ourINFO)[] {
                ...
                // === FRIENDSHIP SCENE NPC VOICE (added 2026-02-03) ===
                // Exchange 1 NPC responses
                (0x1658C5, friend_ex1_NPos.Responses[0].FormKey),  // "Seems like you're doing better..."
                (0x16599B, friend_ex1_NNeg.Responses[0].FormKey),  // "Oh yeah, sure. I'm just talking."
                (0x165955, friend_ex1_NNeu.Responses[0].FormKey),  // "So this is you not even trying, huh?"
                (0x165911, friend_ex1_NQue.Responses[0].FormKey),  // "Just the ones I find particularly interesting."
                // Dialog 2
                (0x1658DB, friend_Dialog2.Responses[0].FormKey),   // "I appreciate it..."
                // Exchange 2 NPC responses
                (0x1659BD, friend_ex2_NPos.Responses[0].FormKey),  // "Heh, no kidding..."
                (0x16596D, friend_ex2_NNeg.Responses[0].FormKey),  // "Because people need to know the truth..."
                (0x16592B, friend_ex2_NNeu.Responses[0].FormKey),  // "What they want?..."
                (0x1658E3, friend_ex2_NQue.Responses[0].FormKey),  // "Are you kidding me?..." (long synth speech)
                // Dialog 4
                (0x1659DF, friend_Dialog4.Responses[0].FormKey),   // "Sure, it can be scary..."
                // Exchange 3 NPC responses
                (0x165982, friend_ex3_NPos.Responses[0].FormKey),  // "Exactly..."
                (0x165940, friend_ex3_NNeg.Responses[0].FormKey),  // (may be silent)
                (0x1658F9, friend_ex3_NNeu.Responses[0].FormKey),  // (may be silent)
                (0x165A1D, friend_ex3_NQue.Responses[0].FormKey),  // "I'm pushy, not crazy..."
                // Dialog 7
                (0x16599E, friend_Dialog7.Responses[0].FormKey),   // "I've seen firsthand..." (father story)
                // Exchange 4 NPC responses
                (0x165956, friend_ex4_NPos.Responses[0].FormKey),  // "No, those people saved themselves..."
                (0x165914, friend_ex4_NNeg.Responses[0].FormKey),  // "But how many more people..."
                (0x1658D1, friend_ex4_NNeu.Responses[0].FormKey),  // "Thanks. It was hard..."
                (0x1659B2, friend_ex4_NQue.Responses[0].FormKey),  // "We made due..."
                // Closing
                (0x16596E, friend_closingTopic.Responses[0].FormKey), // "But, hey, I-I'm sorry if I've been rambling..."
            };
            ...
            var playerVoiceMap = new (uint piperINFO, FormKey ourINFO)[] {
                ...
                // === FRIENDSHIP SCENE PLAYER VOICE (added 2026-02-03) ===
                // Exchange 1
                (0x1658D0, friend_ex1_PPos.Responses[0].FormKey),  // "I try to be."
                (0x1659AF, friend_ex1_PNeg.Responses[0].FormKey),  // "Maybe we can have this chat later?"
                (0x165963, friend_ex1_PNeu.Responses[0].FormKey),  // "Didn't realize I had been."
                (0x16591D, friend_ex1_PQue.Responses[0].FormKey),  // "You always pry this much..."
                // Exchange 2
                (0x1659CF, friend_ex2_PPos.Responses[0].FormKey),  // "Sounds like it must be exhausting."
                (0x165975, friend_ex2_PNeg.Responses[0].FormKey),  // "Then quit..."
                (0x165935, friend_ex2_PNeu.Responses[0].FormKey),  // "Why not just give people what they want?"
                (0x1658ED, friend_ex2_PQue.Responses[0].FormKey),  // "Is Diamond City really in danger?"
                // Exchange 3
                (0x165990, friend_ex3_PPos.Responses[0].FormKey),  // "Couldn't agree more..."
                (0x16594B, friend_ex3_PNeg.Responses[0].FormKey),  // "The truth creates fear, suspicion..."
                (0x165907, friend_ex3_PNeu.Responses[0].FormKey),  // "You'd really risk your family like that?"
                (0x1658C6, friend_ex3_PQue.Responses[0].FormKey),  // "Scared, huh? Could've fooled me."
                // Exchange 4
                (0x165964, friend_ex4_PPos.Responses[0].FormKey),  // "You saved those people."
                (0x165920, friend_ex4_PNeg.Responses[0].FormKey),  // "Your father died for knowing the truth..."
                (0x1658DC, friend_ex4_PNeu.Responses[0].FormKey),  // "I'm sorry about your dad."
                (0x1659C0, friend_ex4_PQue.Responses[0].FormKey),  // "What happened after that?"
            };
